...
from backend.models import Recurso
...
def actualizar_unidad(rid: str, data: UnidadIn, db: Session = Depends(get_db)):
    r = db.query(Recurso).get(rid)
    if not r:
        raise HTTPException(404, "Recurso no encontrado")
...
def actualizar_precio(rid: str, data: PrecioIn, db: Session = Depends(get_db)):
    r = db.query(Recurso).get(rid)
    if not r:
        raise HTTPException(404, "Recurso no encontrado")
