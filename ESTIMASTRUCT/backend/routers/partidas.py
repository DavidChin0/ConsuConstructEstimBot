...
def crear(cid: str, data: PartidaIn, db: Session = Depends(get_db)):
    ...
    base = calc_base(data.costo_mo, data.costo_ma, data.unitario_matriz)
...
def _norm_csi(s: str) -> str:
    """Normaliza clave CSI para comparar: minuscula, espacios colapsados, sin guiones."""
    return re.sub(r"\s+", " ", (s or "").replace("-", " ")).strip().lower()


@router.get("/partidas/by-csi/{clave_csi:path}")
def por_csi(clave_csi: str, presupuesto_id: Optional[str] = None,
            db: Session = Depends(get_db)):
    """
    Busca partidas por clave CSI (keynote del visor 3D).
    Matching: exacto -> normalizado -> prefijo (tolera precision extra del keynote).
    Filtro opcional por presupuesto_id (via Capitulo).
    """
    q = db.query(Partida)
    if presupuesto_id:
        q = q.join(Capitulo, Partida.capitulo_id == Capitulo.id).filter(
            Capitulo.presupuesto_id == presupuesto_id
        )
    todas = q.all()

    target = _norm_csi(clave_csi)
    if not target:
        return []

    exactas, prefijo = [], []
    for p in todas:
        c = _norm_csi(p.clave_csi)
        if not c:
            continue
        if c == target:
            exactas.append(p)
    ...
    hits = exactas or prefijo
    return [_partida_dict(p) for p in hits]
