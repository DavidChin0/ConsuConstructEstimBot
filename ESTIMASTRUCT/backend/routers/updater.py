...
UPDATER_PATH = CONFIG.UPDATER_DIR
...
def _safe_within(base: str, *parts: str) -> str:
    """Une parts a base y garantiza que el resultado queda DENTRO de base (anti path traversal)."""
...
def import_fichas(req: ImportRequest):
    if not re.match(r"^[A-Za-z0-9_.\-]+$", req.version or ""):
        raise HTTPException(400, "Versión inválida")
    filepath = _safe_within(UPDATER_PATH, os.path.basename(req.filename))
...
def _sync_new_fichas_to_db(fichas: list, only_codigos: list, version: str, db: Session) -> int:
    ...
    for cfg in configs:
        ...
        for codigo, fi in ficha_map.items():
            ...
            # Compute costs from insumos — bucketing 3-vias por _tipo_from_clave (prefijo con
            # guion: "MO-"/"MO.", no "MO" pelado, que atrapaba falsos positivos tipo "MOR-").
            mo, ma, matriz = 0.0, 0.0, 0.0
